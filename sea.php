        ...
        const app = {
            ...
            async ajax(action, data = {}) {
                ...
                try {
                    ...
                    // Polyglot guard: GIF header before opening PHP tag leaks into output
                    // when output_buffering is Off. Search for __ajax sentinel
                    // instead of requiring it at position 0.
                    const ajaxIdx = trimmed.indexOf('__ajax');
                    if (ajaxIdx === -1) {
                        this.showLoading(false);
                    ...
                    let result = JSON.parse(trimmed.substring(ajaxIdx + 6));
                    this.showLoading(false);
                    return result;
                } catch (err) {
                    this.showLoading(false);
